...
int main() {
    ...
    string ex = "\"MGTToodDOkkkNUTM\",\n"
                "\"MisssssissipppiiPPippi\",\n"
                "\"GFeedCAaaaaaaaAbBbCddeFG\"";
    string main = "\"zzvvJJaOkuIPPIQQhodakSSjjjXXaFyyyujiKitvz\",\n"
...
void solve(){
    ...
    while (true){
        ...
        rep(i, 0, T.size()){
            ...
            && 'A' <= T[i] && T[i] <= 'Z'){
                ok = true;
                rep(j, 0, 2) s.insert((i + j) % (int)S.size());
                break;
            }
            else if (i + 2 < (int)T.size()
            && T[i] == T[i + 1] && T[i + 1] == T[i + 2]
            && 'a' <= T[i] && T[i] <= 'z'){
                ok = true;
                rep(j, 0, 3) s.insert((i + j) % (int)S.size());
                break;
            }
        }
        string nS;
